...
const MULTICHANNEL_PATH = '/admin/multi-channel';
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl text-white font-semibold">LinkedIn Campaigns</h1>
          <p className="text-sm text-gray-400">{userEmail ? `Signed in as ${userEmail}` : 'Please sign in.'}</p>
        </div>
        <button className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm" type="button">
      ...
      <div className="bg-[#0b0f14] border border-gray-800 rounded-xl p-5">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs text-gray-400">
                    Optimization hint <span className="text-gray-500">(costs 1 credit)</span>
                  </label>
                </div>
              ...
              <div className="border border-gray-800 rounded-lg p-4 space-y-3">
                ...
                <div className="flex gap-2 text-xs">
                  <button
                    ...
                    className={`px-2 py-1 rounded border ${imageMode==='upload'?'border-emerald-500 text-emerald-400':'border-gray-700 text-gray-400'}`}
                  >
                    Upload
                  </button>
                  <button
                    ...
                    className={`px-2 py-1 rounded border ${imageMode==='ai'?'border-emerald-500 text-emerald-400':'border-gray-700 text-gray-400'}`}
                  >
                    Generate with AI
                  </button>
                </div>
              ...
              <div className="border border-gray-800 rounded-lg p-4 space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    {postEnabled ? 'Post to LinkedIn' : (liConnected ? 'Complete sign-in & type a post' : 'Connect LinkedIn')}
                  </button>
                </div>
                {liConnected && !canPost && (
        ...
        {ENABLE_FACEBOOK && activeTab==='Facebook' && (
          <FacebookPanel supabase={supabase} callFn={callFn} refreshWallet={refreshWallet} />
        )}
        {ENABLE_EMAIL && activeTab==='Email' && (
          <div className="text-sm text-gray-400">Email composer coming soon.</div>
        )}
        {ENABLE_PHONE && activeTab==='Phone' && (
          <div className="text-sm text-gray-400">Phone/SMS composer coming soon.</div>
        )}
