  ...
  useEffect(() => {
    (async () => {
      ...
      if (error || !data?.verified) {
        setStatus('Verification failed. If amount was deducted, we will auto-credit after webhook.');
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 flex items-center justify-center p-8">
      <div className="max-w-lg text-center">
        <h1 className="text-3xl font-bold text-emerald-400">Checkout Success</h1>
        <p className="text-gray-300 mt-2">{status}</p>

        {credited > 0 && (
          <p className="text-sm text-gray-400 mt-3">
            Added <span className="text-emerald-300 font-semibold">{credited}</span> credits to your wallet.
            {balance != null && <> New balance: <span className="text-emerald-300 font-semibold">{balance}</span>.</>}
          </p>
        )}
